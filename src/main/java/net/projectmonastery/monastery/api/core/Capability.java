...
public interface Capability {
     ...
     * <p>This allows the capability to access dependencies, should the capability depend on other capabilities.</p>
     * <p><em>Note that at the time of binding, there is no assurance that the other capabilities have been added to the node,
     ...
     * <p>At this stage it is possible for the capability to check whether all the dependencies it has are bound to the
     * node and it has the opportunity to configure itself appropriately, or throw an exception if it concludes that
     * it cannot function.</p>
     * <p><em>Note that aqt this stage the node is probably not joined to the cluster yet, so it would be premature to start
     * any activity that requires any access to the cluster.</em></p>
     */
